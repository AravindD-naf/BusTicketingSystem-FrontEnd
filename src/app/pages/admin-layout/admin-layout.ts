...
@Component({
    ...
    <div class="admin-shell">
      <aside class="sidebar" [class.collapsed]="collapsed()">
        ...
        <nav class="nav">
          ...
          <a routerLink="/admin/chat" routerLinkActive="active" class="nav-link chat-nav-link">
            <span>💬</span>
            <span *ngIf="!collapsed()" class="label">Messages</span>
            <span *ngIf="chatService.totalUnread() > 0" class="unread-badge">{{ chatService.totalUnread() }}</span>
          </a>
        </nav>
  ...
  styles: [`
    ...
    .toggle-btn{position:absolute;top:14px;right:-14px;width:28px;height:28px;background:#1e3a6e;border:2px solid rgba(255,255,255,0.15);border-radius:50%;color:#fff;cursor:pointer;font-size:.85rem;display:flex;align-items:center;justify-content:center;z-index:20;transition:background .2s;box-shadow:0 2px 6px rgba(0,0,0,.3);}
    ...
    .unread-badge{background:#ef4444;color:white;border-radius:20px;font-size:11px;font-weight:700;padding:2px 7px;flex-shrink:0;min-width:20px;text-align:center;}
