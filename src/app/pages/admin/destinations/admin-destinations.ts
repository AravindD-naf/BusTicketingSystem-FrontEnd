        ...
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <form [formGroup]="destForm" (ngSubmit)="saveDestination()" class="modal-form">
            ...
            <div class="modal-actions">
              ...
              <button type="submit" class="btn-primary" [disabled]="saving()">{{ saving() ? 'Saving...' : (editingId() ? 'Update' : 'Add Destination') }}</button>
            </div>
...
export class AdminDestinations implements OnInit {
  ...
  destForm = this.fb.group({ destinationName: ['', Validators.required], description: [''], isActive: [true] });

  ngOnInit() { this.loadDestinations(); }

  loadDestinations() {
    this.loading.set(true);
    this.destService.getAllDestinations(1, 200).subscribe({
      next: r => { this.destinations.set(Array.isArray(r.data) ? r.data : []); this.loading.set(false); },
      error: e => { this.error.set(e?.error?.message || 'Failed to load destinations'); this.loading.set(false); }
    });
  }

  openModal() { this.destForm.reset({ destinationName:'', description:'', isActive:true }); this.editingId.set(null); this.formError.set(null); this.showModal.set(true); }
  ...
  saveDestination() {
    ...
    const req$ = id ? this.destService.updateDestination(id, v) : this.destService.createDestination(v);
    req$.subscribe({
      next: () => { this.saving.set(false); this.closeModal(); this.loadDestinations(); },
  ...
  deleteDestination(id: number) {
    ...
    this.destService.deleteDestination(id).subscribe({ next: () => this.loadDestinations(), error: e => alert(e?.error?.message || 'Delete failed') });
  }
}
