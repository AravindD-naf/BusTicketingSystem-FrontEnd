...
//                 <button class="btn-refund" *ngIf="b.bookingStatus === 'Cancelled' && getRefund(b.bookingId)" (click)="openRefundModal(b)">💰 Refund</button>
//               </td>
//             </tr>
...
//             <input type="hidden" formControlName="refundId" />
//             <div class="refund-detail-row">
//               <span>Refund Amount</span>
//               <strong style="color:#16a34a">₹{{ selectedRefundBooking()?.refundAmount }}</strong>
//             </div>
//             <div class="refund-detail-row">
//               <span>Cancellation Fee</span>
//               <strong style="color:#dc2626">₹{{ selectedRefundBooking()?.cancellationFee }}</strong>
//             </div>
//             <div class="refund-detail-row">
//               <span>Policy Applied</span>
//               <span>{{ selectedRefundBooking()?.refundPercentage }}% refund</span>
//             </div>
...
//   selectedRefundBooking = signal<any>(null);
//   searchQuery     = '';
//   statusFilter    = '';
...
//         this.applyFilter();
//         const cancelled = data.filter((b: any) => b.bookingStatus === 'Cancelled');
//         cancelled.forEach((b: any) => this.loadRefundForBooking(b.bookingId));
...
//     const refund = this.getRefund(b.bookingId);
//     if (!refund) {
//       alert('No refund record found for this booking.');
//       return;
//     }
//     this.selectedRefundBooking.set(refund);
...
@Component({
    ...
    <div>
          ...
          <tbody>
            <tr *ngFor="let b of filteredBookings(); let i = index">
              ...
              <td>
                ...
                <button class="btn-cancel" *ngIf="b.bookingStatus === 'Confirmed'" (click)="cancelBooking(b.bookingId)">❌ Cancel</button>
                <!-- Refund button for cancelled bookings that have a refund record -->
                <button class="btn-refund" *ngIf="b.bookingStatus === 'Cancelled' && getRefund(b.bookingId)" (click)="openRefundModal(b)">💰 Refund</button>
              </td>
            </tr>
            <tr *ngIf="filteredBookings().length === 0">
      ...
      <div class="modal-overlay" *ngIf="selectedBooking()" (click)="selectedBooking.set(null)">
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <div class="detail-grid">
            ...
            <!-- Cancellation & Refund Info — only for cancelled bookings -->
            <ng-container *ngIf="selectedBooking()?.bookingStatus === 'Cancelled'">
              <div class="detail-section-title">Cancellation Info</div>
              <div class="detail-row">
                <span class="detail-key">Cancelled By</span>
                <span>{{ selectedBooking()?.cancelledBy || '—' }}</span>
              </div>

              <!-- Has refund record (was a paid/confirmed booking) -->
              <ng-container *ngIf="getRefund(selectedBooking()?.bookingId)">
                <div class="detail-section-title">Refund Details</div>
                <div class="detail-row">
                  <span class="detail-key">Refund Amount</span>
                  <span style="color:#16a34a;font-weight:700">
                    ₹{{ getRefund(selectedBooking()?.bookingId)?.refundAmount }}
                  </span>
                </div>
                <div class="detail-row">
                  <span class="detail-key">Cancellation Fee</span>
                  <span style="color:#dc2626">
                    ₹{{ getRefund(selectedBooking()?.bookingId)?.cancellationFee }}
                  </span>
                </div>
                <div class="detail-row">
                  <span class="detail-key">Policy Applied</span>
                  <span>{{ getRefund(selectedBooking()?.bookingId)?.refundPercentage }}% of fare refunded</span>
                </div>
                <div class="detail-row">
                  <span class="detail-key">Refund Status</span>
                  <span [class]="getRefundStatusClass(getRefund(selectedBooking()?.bookingId)?.status)">
                    {{ getRefund(selectedBooking()?.bookingId)?.status }}
                  </span>
                </div>
                <div class="detail-row" *ngIf="getRefund(selectedBooking()?.bookingId)?.processedAt">
                  <span class="detail-key">Processed On</span>
                  <span>{{ getRefund(selectedBooking()?.bookingId)?.processedAt | date:'mediumDate' }}</span>
                </div>
              </ng-container>

              <!-- No refund record — booking was not paid before cancellation -->
              <div class="detail-row" *ngIf="!getRefund(selectedBooking()?.bookingId)">
                <span class="detail-key">Refund</span>
                <span style="color:#94a3b8">Not applicable — no payment was made.</span>
              </div>
            </ng-container>
      ...
      <div class="modal-overlay" *ngIf="showRefundModal()" (click)="showRefundModal.set(false)">
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <!-- Refund summary — read-only display -->
          <div class="refund-summary-box" *ngIf="selectedRefundBooking()">
            <div class="refund-summary-row">
              <span>Booking</span>
              <strong>#{{ selectedRefundBooking()?.bookingId }}</strong>
            </div>
            <div class="refund-summary-row">
              <span>Cancelled By</span>
              <span>{{ selectedRefundBooking()?.cancelledBy }}</span>
            </div>
            <div class="refund-summary-row">
              <span>Refund Amount</span>
              <strong style="color:#16a34a">₹{{ selectedRefundBooking()?.refundAmount }}</strong>
            ...
            <div class="refund-summary-row">
              <span>Cancellation Fee</span>
              <span style="color:#dc2626">₹{{ selectedRefundBooking()?.cancellationFee }}</span>
            </div>
            <div class="refund-summary-row">
              <span>Policy Applied</span>
              <span>{{ selectedRefundBooking()?.refundPercentage }}% refund</span>
            ...
            <div class="refund-summary-row">
              ...
              <span [class]="getRefundStatusClass(selectedRefundBooking()?.status)">
                {{ selectedRefundBooking()?.status }}
              </span>
            </div>
          </div>

          <form [formGroup]="refundForm" (ngSubmit)="confirmRefund()" class="modal-form" style="margin-top:16px">
            <!-- Hidden refundId — auto-filled from selectedRefundBooking -->
            <input type="hidden" formControlName="refundId" />
            ...
            <label class="form-label">Reason / Notes
              <input class="form-input" formControlName="reason" placeholder="Optional reason for this decision" />
            </label>
            <div class="modal-actions">
              <button type="button" class="btn-secondary" (click)="showRefundModal.set(false)">Cancel</button>
  ...
  styles: [`
    ...
    .status-failed{background:#fce7f3;color:#9d174d}
    .status-default{background:#f1f5f9;color:#64748b}
    .refund-pending{background:#fef9c3;color:#ca8a04;padding:3px 10px;border-radius:20px;font-size:.75rem;font-weight:500}
    .refund-processing{background:#eff6ff;color:#1d4ed8;padding:3px 10px;border-radius:20px;font-size:.75rem;font-weight:500}
    .refund-completed{background:#dcfce7;color:#16a34a;padding:3px 10px;border-radius:20px;font-size:.75rem;font-weight:500}
    .refund-rejected{background:#fee2e2;color:#dc2626;padding:3px 10px;border-radius:20px;font-size:.75rem;font-weight:500}
    .tax-note{font-size:.7rem;color:#64748b;margin-left:2px;}
    .empty-row{text-align:center;color:#94a3b8;padding:32px!important}
    .info-box{padding:12px 16px;background:#eff6ff;border-radius:8px;color:#1d4ed8;font-size:.875rem;margin-bottom:16px}
    .error-box{padding:12px 16px;background:#fee2e2;border-radius:8px;color:#dc2626;font-size:.875rem;margin-bottom:16px}
    ...
    .form-input:focus{border-color:#3b82f6}
    .modal-actions{display:flex;gap:10px;justify-content:flex-end;margin-top:4px}
    .refund-summary-box{background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:14px 16px;display:flex;flex-direction:column;gap:8px;}
    .refund-summary-row{display:flex;justify-content:space-between;font-size:.875rem;color:#374151;}
    .refund-summary-row span:first-child{color:#64748b;}
    .results-summary{font-size:.82rem;color:#64748b;margin:12px 0 8px;text-align:center;}
    .results-summary strong{color:#0A1F44;}
    .pagination-bar{display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:nowrap;margin-top:8px;padding:12px 0;}
    .page-btn{background:#fff;border:1.5px solid #e2e8f0;border-radius:8px;padding:7px 14px;font-size:.8rem;font-weight:600;color:#0A1F44;cursor:pointer;transition:all .18s;white-space:nowrap;}
...
export class AdminBookings implements OnInit {
  ...
  // Refund data map: bookingId → refund object
  refundData = signal<Map<number, any>>(new Map());
  ...
  refundForm = this.fb.group({ refundId: [null as number | null], isApproved: [true], reason: [''] });
  ...
  loadBookings() {
    ...
    this.bookingService.getAllBookings(this.currentPage(), this.pageSize()).subscribe({
      next: (r: any) => {
        ...
        this.applyFilter();
        this.loading.set(false);
        // Load refund data for all cancelled bookings on this page
        data
          .filter((b: any) => b.bookingStatus === 'Cancelled')
          .forEach((b: any) => this.loadRefundForBooking(b.bookingId));
      },
      error: e => { this.error.set(e?.error?.message || 'Failed to load bookings'); this.loading.set(false); }
    });
  }
  ...
  loadRefundForBooking(bookingId: number) {
    this.paymentService.getRefundByBooking(bookingId).subscribe({
      ...
      error: () => {} // silently ignore — no refund record exists
    });
  }
  ...
  applyFilter() {
    let list = this.bookings();
    if (this.searchQuery) list = list.filter(b =>
      String(b.bookingId).includes(this.searchQuery) ||
      b.bookingStatus?.toLowerCase().includes(this.searchQuery)
    );
    if (this.statusFilter) list = list.filter(b => b.bookingStatus === this.statusFilter);
    this.filteredBookings.set(list);
  }
  ...
  viewBooking(b: any) {
    ...
    this.bookingService.getBookingById(b.bookingId).subscribe({
      next: (r: any) => {
        this.selectedBooking.set({ ...b, ...(r?.data ?? r) });
        this.detailLoading.set(false);
        // Also load refund if cancelled and not already loaded
        if (b.bookingStatus === 'Cancelled' && !this.getRefund(b.bookingId)) {
  ...
  cancelBooking(id: number) {
    if (!confirm('Cancel this booking? A refund will be automatically calculated based on the cancellation policy.')) return;
    this.bookingService.cancelBooking(id).subscribe({
      next: () => { this.currentPage.set(1); this.loadBookings(); },
      error: e => alert(e?.error?.message || 'Cancel failed')
    });
  ...
  openRefundModal(b: any) {
    ...
    // Store the full context (booking + refund merged) for display
    this.selectedRefundBooking.set({ ...b, ...refund });
    // Auto-fill the hidden refundId — admin only picks approve/reject + reason
    this.refundForm.reset({ refundId: refund.refundId, isApproved: true, reason: '' });
    this.formError.set(null);
    this.showRefundModal.set(true);
  }
  ...
  confirmRefund() {
    ...
    this.paymentService.confirmRefund({
      ...
      error: e => { this.saving.set(false); this.formError.set(e?.error?.message || 'Refund processing failed'); }
    });
  }
