        ...
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-row">
              ...
              <span [class]="getStatusClass(selectedBooking()?.bookingStatus)">{{ getStatusLabel(selectedBooking()?.bookingStatus) }}</span>
            </div>
            <div class="detail-row"><span class="detail-key">Booked On</span><span>{{ selectedBooking()?.bookingDate | date:'mediumDate' }}</span></div>
            ...
            <div class="detail-row"><span class="detail-key">Departure</span><span>{{ selectedBooking()?.departureTime || '—' }}</span></div>
            <div class="detail-row"><span class="detail-key">Arrival</span><span>{{ selectedBooking()?.arrivalTime || '—' }}</span></div>
            ...
            <div class="detail-row"><span class="detail-key">Bus Type</span><span>{{ selectedBooking()?.busType || '—' }}</span></div>
            ...
            <ng-container *ngIf="selectedBooking()?.bookingStatus === 'Cancelled'">
              ...
              <ng-container *ngIf="selectedBooking()?.refund">
                ...
                <div class="detail-row">
                  ...
                  <span style="color:#16a34a;font-weight:700">₹{{ selectedBooking()?.refund?.refundAmount }}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-key">Cancellation Fee</span>
                  <span style="color:#dc2626">₹{{ selectedBooking()?.refund?.cancellationFee }}</span>
                </div>
                <div class="detail-row"><span class="detail-key">Policy Applied</span><span>{{ selectedBooking()?.refund?.refundPercentage }}% of fare refunded</span></div>
                <div class="detail-row">
                  <span class="detail-key">Refund Status</span>
                  <span [class]="getRefundStatusClass(selectedBooking()?.refund?.status)">{{ selectedBooking()?.refund?.status }}</span>
                </div>
                <div class="detail-row" *ngIf="selectedBooking()?.refund?.processedAt">
                  <span class="detail-key">Processed On</span>
                  <span>{{ selectedBooking()?.refund?.processedAt | date:'mediumDate' }}</span>
                </div>
              </ng-container>
              <div class="detail-row" *ngIf="!selectedBooking()?.refund">
      ...
      <div class="modal-overlay" *ngIf="showRefundModal()" (click)="showRefundModal.set(false)">
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <div class="refund-summary-box" *ngIf="selectedRefundBooking()">
            <div class="refund-summary-row"><span>Booking</span><strong>#{{ selectedRefundBooking()?.bookingId }}</strong></div>
            <div class="refund-summary-row"><span>Cancelled By</span><span>{{ selectedRefundBooking()?.cancelledBy }}</span></div>
            <div class="refund-summary-row"><span>Refund Amount</span><strong style="color:#16a34a">₹{{ selectedRefundBooking()?.refund?.refundAmount }}</strong></div>
            <div class="refund-summary-row"><span>Cancellation Fee</span><span style="color:#dc2626">₹{{ selectedRefundBooking()?.refund?.cancellationFee }}</span></div>
            <div class="refund-summary-row"><span>Policy Applied</span><span>{{ selectedRefundBooking()?.refund?.refundPercentage }}% refund</span></div>
            <div class="refund-summary-row">
              <span>Current Status</span>
              <span [class]="getRefundStatusClass(selectedRefundBooking()?.refund?.status)">{{ selectedRefundBooking()?.refund?.status }}</span>
            </div>
          </div>
          <form [formGroup]="refundForm" (ngSubmit)="confirmRefund()" class="modal-form" style="margin-top:16px">
            ...
            <div class="modal-actions">
              ...
              <button type="submit" class="btn-primary" [disabled]="saving()">{{ saving() ? 'Processing...' : 'Submit Decision' }}</button>
            </div>
...
export class AdminBookings implements OnInit {
  ...
  viewBooking(b: any) {
    ...
    this.bookingService.getBookingById(b.bookingId).subscribe({
      next: (r: any) => {
        this.selectedBooking.set({ ...b, ...(r?.data ?? r), refund: b.refund });
